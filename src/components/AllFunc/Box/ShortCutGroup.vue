<template>
    ...
    <div class="shortcut__layout">
        <Transition name="fade" mode="out-in">
            <n-scrollbar v-if="shortcutGroupData.length" class="shortcut">
                <n-collapse v-for="categoryData in shortcutGroupData" :key="categoryData.categoryName"
                    arrow-placement="right" class="collapse-content">
                    <!-- <template #header-extra>
                        <span v-if="isShowEditCategoryNameBtn" class="icon_span--style icon--padding6"
                            @click.stop="handleOpenEditCategoryName(categoryData.categoryName)">
                            <el-icon style="vertical-align: middle">
                                <Edit />
                            </el-icon>
                        </span>
                        <span v-if="isShowDeleteCategoryNameBtn" class="icon_span--style icon--padding6"
                    ...
                    <template #arrow>
                        <SvgIcon iconName="icon-xianxingshuangzhixiangxiahuadong"
                    ...
                    <n-collapse-item :name="categoryData.categoryName">
                        ...
                        <n-scrollbar class="scrollbar">
                            <n-grid class="all-shortcut" responsive="screen" cols="2 s:3 m:4 l:5" :x-gap="10"
                                :y-gap="10">
                                <n-grid-item v-for="item in categoryData.websiteDataList" :key="item"
                                    class="shortcut-item" @contextmenu="shortCutContextmenu($event, item)"
    ...
    <n-modal preset="card" v-model:show="addShortcutModalShow" :title="`${addShortcutModalType ? '编辑' : '添加'}捷径`"
        ...
        <n-form ref="addShortcutRef" :rules="addShortcutRules" :model="addShortcutValue" :label-width="80">
            ...
            <n-form-item label="捷径名称" path="name">
                <n-input clearable show-count maxlength="14" v-model:value="addShortcutValue.name"
                    placeholder="请输入捷径名称" />
            </n-form-item>
...
import {
    ...
    NInput,
    NInputNumber,
    NDropdown, NCollapse, NCollapseItem
} from "naive-ui";
import { storeToRefs } from "pinia";
import { siteGroupStore, setStore } from "@/stores";
import SvgIcon from "@/components/SvgIcon.vue";
import identifyInput from "@/utils/identifyInput";
import { onMounted, onBeforeUnmount } from "vue";

const set = setStore();
const site = siteGroupStore();
const { shortcutGroupData } = storeToRefs(site);
// 图标渲染
...
const addShortcutModalOpen = () => {
    // 生成 ID
    const shortcutMaxID = categoryDataList.value.reduce((max, item) => {
